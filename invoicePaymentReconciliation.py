...
def reconcile_payment(payment:str, invoices:List[str])-> str:
    """
      Input
    payment="payment_id,amount, Paying off:{INVOICE_ID}"
    invoices=["invoice_id, due_date,amount_due"]
    Output:
    Output: payment5 pays off 1000 for invoiceC due on 2023-01-30
    """
    payment_parts=payment.split(",")
    payment_id=payment_parts[0].strip()
    payment_amount=int(payment_parts[1].strip())
    memo=payment_parts[2].strip()
    invoice_id=extract_invoice_id_from_memo(memo)
    if not invoice_id:
        return f"Error:Could not extract invoice ID from memo: {memo}"
    invoice_data=find_invoice(invoice_id,invoices)
    if not invoice_data:
        return f"Error:Invoice {invoice_id} was not found "
    return format_reconciliation_result(payment_id, payment_amount,invoice_data)
...
def extract_invoice_id_from_memo(memo):
    if "Paying off:" in memo:
        ...
        return invoice_id
    return None
def find_invoice(invoice_id, invoices:List[str]):
...
def format_reconciliation_result(payment_id,payment_amount, invoice):
    # def format_reconciliation(payment_id: str, amount: int, invoice: Dict) -> str:
    return (f"{payment_id} pays_off {payment_amount} for {invoice["invoice_id"]}"
            f" due on {invoice["due_date"]}")

#Test case 
payment="payment5, 100, Paying off: invoiceC"
invoices=["invoceA, 2024-12-09,100","invoceB, 2024-12-09, 100", "invoiceC, 2024-12-10, 100" ]
